...
from app.services.ai_parser import extract_entities, detect_relationships, generate_embeddings_batch, map_entities_to_timestamps

logger = logging.getLogger(__name__)


async def process_note_pipeline(note_id: int, db: AsyncSession, transcript_segments: Optional[List[Dict[str, Any]]] = None) -> AsyncGenerator[str, None]:
    """
    ...
    try:
        ...
        logger.info(f"Processing note {note_id}: {note_title}")
        yield send_event("start", f"Processing: {note_title}", 0)
        
        # Step 1: Extract entities
        logger.info("Step 1: Extracting entities...")
        yield send_event("extracting", "Analyzing text and extracting entities...", 20)
        entities_data = await extract_entities(note_content)
        ...
        texts_to_embed = [note_content]  # First item is the note
        ...
        for idx, entity_data in enumerate(entities_data):
            ...
            entity_timestamp = entity_timestamps.get(entity_name)
        ...
        yield send_event("saving", f"Saved {saved_entities_count} entities", 70)
        ...
        relationships_data = await detect_relationships(entities_data, note_content)
        ...
        for rel_data in relationships_data:
            ...
            db.add(relationship)
            saved_relationships_count += 1
        
        # Commit everything
        await db.commit()
        
        logger.info(f"✅ Processing complete: {saved_entities_count} entities, {saved_relationships_count} relationships")
        yield send_event("complete", f"Processing complete! {saved_entities_count} entities, {saved_relationships_count} relationships", 100)
