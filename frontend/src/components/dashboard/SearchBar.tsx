...
export default function SearchBar({
  onSearch,
  onYouTubeProcess,
  placeholder = 'FIND_CONCEPT_OR_TECHNOLOGY...',
}: SearchBarProps) {
      ...
      <form onSubmit={handleSubmit} className="w-full">
        <div className="relative group">
          <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
            {mode === 'search' ? (
              <svg className="w-4 h-4 text-slate-600 group-focus-within:text-blue-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input
            ...
            `}
            placeholder={mode === 'search' ? placeholder : 'PASTE_EXTERNAL_STREAM_QUERY...'}
          />
          ...
          {query && (
            <button
              type="button"
              onClick={handleClear}
              className="absolute inset-y-0 right-4 flex items-center text-slate-700 hover:text-white transition-colors"
            >
        ...
        <div className="mt-6 flex flex-wrap gap-3 items-center">
          <span className="text-[8px] uppercase tracking-[0.3em] font-black text-slate-700 font-mono pr-2">
            PRESETS:
          </span>
          {(mode === 'search'
            ? ['Distributed Systems', 'Graph Theory', 'Neural Networks']
            ...
            <button
              ...
              className="
                text-[9px] px-3 py-1 font-mono font-bold uppercase
